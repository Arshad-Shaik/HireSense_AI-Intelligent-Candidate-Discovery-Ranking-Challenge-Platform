...
:root[data-theme="dark"] {
    ...
    --holo-surface: #0a1628;
    --holo-text: #ffffff;
    --holo-text-muted: rgba(0,212,255,0.7);
    --holo-text-sub: rgba(180,130,255,0.9);
    --holo-border: rgba(0,212,255,0.08);
    --holo-card-bg: linear-gradient(135deg, rgba(0,212,255,0.12) 0%, rgba(123,47,247,0.12) 100%);
    --holo-card-border: rgba(0,212,255,0.35);
    --holo-card-shadow: 0 0 20px rgba(0,212,255,0.2);
    --holo-input-bg: rgba(10,22,40,0.9);
    --holo-input-border: rgba(0,212,255,0.3);
    --holo-df-bg: #0a1628;
    --holo-df-row-hover: rgba(0,212,255,0.07);
    --holo-df-row-border: rgba(0,212,255,0.06);
    --holo-footer-color: rgba(0,212,255,0.5);
    --holo-footer-border: rgba(0,212,255,0.08);
    --holo-logo-bg: #0a1628;
    --holo-subtitle-color: rgba(0,212,255,0.9);
    --holo-radial1: rgba(123,47,247,0.08);
    --holo-radial2: rgba(0,212,255,0.06);
    --holo-radial3: rgba(0,255,136,0.04);
    --holo-grid-line: rgba(0,212,255,0.03);
    --holo-scrollbar-track: rgba(0,212,255,0.05);
    --holo-scrollbar-thumb: rgba(0,212,255,0.4);
    --holo-tab-border: rgba(0,212,255,0.2);
    --holo-tab-color: rgba(0,212,255,0.8);
    --holo-label-color: rgba(0,212,255,0.9);
    --holo-theme-btn-bg: rgba(0,212,255,0.1);
    --holo-theme-btn-border: rgba(0,212,255,0.4);
    --holo-theme-btn-color: #00d4ff;

    /* Map Gradio's native theme variables onto our theme so
       built-in Gradio components (not just our custom classes)
       also follow the toggle instead of staying stuck on
       whatever Gradio decided at page load. */
    --body-background-fill: var(--holo-bg);
    --background-fill-primary: var(--holo-surface);
    --background-fill-secondary: var(--holo-surface);
    --block-background-fill: var(--holo-surface);
    --panel-background-fill: var(--holo-surface);
    --body-text-color: var(--holo-text);
    --body-text-color-subdued: var(--holo-text-muted);
    --input-background-fill: var(--holo-input-bg);
    --border-color-primary: var(--holo-border);
}

/* Light Theme */
:root[data-theme="light"] {
    --holo-primary: #0077aa;
    --holo-secondary: #5b1faa;
    --holo-accent: #00aa55;
    --holo-gold: #cc9900;
    --holo-bg: #f0f4ff;
    --holo-surface: #ffffff;
    --holo-text: #0a1628;
    --holo-text-muted: rgba(0,100,180,0.85);
    --holo-text-sub: rgba(80,30,160,0.85);
    --holo-border: rgba(0,120,200,0.12);
    --holo-card-bg: linear-gradient(135deg, rgba(0,150,220,0.08) 0%, rgba(100,40,200,0.08) 100%);
    --holo-card-border: rgba(0,120,200,0.3);
    --holo-card-shadow: 0 0 20px rgba(0,120,200,0.15);
    --holo-input-bg: rgba(255,255,255,0.95);
    --holo-input-border: rgba(0,120,200,0.3);
    --holo-df-bg: #ffffff;
    --holo-df-row-hover: rgba(0,120,200,0.06);
    --holo-df-row-border: rgba(0,120,200,0.08);
    --holo-footer-color: rgba(0,100,180,0.6);
    --holo-footer-border: rgba(0,120,200,0.15);
    --holo-logo-bg: #e8f0ff;
    --holo-subtitle-color: rgba(0,100,180,0.9);
    --holo-radial1: rgba(100,40,200,0.05);
    --holo-radial2: rgba(0,120,200,0.04);
    --holo-radial3: rgba(0,160,100,0.03);
    --holo-grid-line: rgba(0,120,200,0.04);
    --holo-scrollbar-track: rgba(0,120,200,0.06);
    --holo-scrollbar-thumb: rgba(0,120,200,0.35);
    --holo-tab-border: rgba(0,120,200,0.2);
    --holo-tab-color: rgba(0,100,180,0.85);
    --holo-label-color: rgba(0,100,180,0.9);
    --holo-theme-btn-bg: rgba(0,120,200,0.08);
    --holo-theme-btn-border: rgba(0,120,200,0.35);
    --holo-theme-btn-color: #0077aa;

    /* Same native-variable mapping as the dark block above,
       resolved against the light values in this scope. */
    --body-background-fill: var(--holo-bg);
    --background-fill-primary: var(--holo-surface);
    --background-fill-secondary: var(--holo-surface);
    --block-background-fill: var(--holo-surface);
...
/* Wrapper — sits at left side to avoid Fullscreen button */
#hs-theme-wrapper {
...
/* The small icon trigger button */
#hs-theme-icon-btn {
    width: 38px !important;
    height: 38px !important;
    border-radius: 50% !important;
    background: var(--holo-theme-btn-bg) !important;
    border: 1.5px solid var(--holo-theme-btn-border) !important;
    color: var(--holo-theme-btn-color) !important;
...
/* Each theme option button inside panel */
.hs-theme-opt {
...
.hs-theme-opt:hover:not(.active) {
    background: rgba(0,212,255,0.12) !important;
}
.hs-theme-opt.active {
    background: linear-gradient(135deg,
        var(--holo-primary),
        var(--holo-secondary)) !important;
...
.gradio-container {
    background: var(--holo-bg) !important;
    background-image:
        radial-gradient(ellipse at 20% 50%, var(--holo-radial1) 0%, transparent 60%),
        radial-gradient(ellipse at 80% 20%, var(--holo-radial2) 0%, transparent 60%),
        radial-gradient(ellipse at 50% 80%, var(--holo-radial3) 0%, transparent 60%) !important;
    min-height: 100vh;
    font-family: 'Inter', 'Segoe UI', sans-serif !important;
    transition: background 0.4s ease, color 0.4s ease !important;
}
...
.hiresense-subtitle {
    color: var(--holo-subtitle-color) !important;
    font-size: 1.1rem;
    margin-top: 8px;
    letter-spacing: 3px;
...
.hiresense-tagline {
    color: var(--holo-text-sub) !important;
    font-size: 0.85rem;
    margin-top: 4px;
    letter-spacing: 1px;
}
...
.stat-card {
    ...
    transition: all 0.3s ease !important;
    box-shadow: var(--holo-card-shadow) !important;
    backdrop-filter: blur(10px) !important;
}
.stat-card:hover {
    border-color: var(--holo-primary) !important;
...
.ranking-scroll [data-testid="dataframe"] > div::-webkit-scrollbar-track {
    background: var(--holo-scrollbar-track) !important;
}
.ranking-scroll [data-testid="dataframe"] > div::-webkit-scrollbar-thumb {
    background: var(--holo-scrollbar-thumb) !important;
    border-radius: 3px !important;
}
.dataframe {
    background: var(--holo-df-bg) !important;
...
.dataframe thead th {
    ...
    border-bottom: 1px solid var(--holo-card-border) !important;
...
.dataframe tbody tr {
    border-bottom: 1px solid var(--holo-df-row-border) !important;
    transition: background 0.2s ease !important;
}
.dataframe tbody tr:hover {
    background: var(--holo-df-row-hover) !important;
}
.dataframe tbody td {
    color: var(--holo-text) !important;
...
input, textarea, select {
    background: var(--holo-input-bg) !important;
    border: 1px solid var(--holo-input-border) !important;
    color: var(--holo-text) !important;
    border-radius: 10px !important;
    transition: all 0.3s ease !important;
}
...
label, .gr-form label {
    color: var(--holo-label-color) !important;
    font-weight: 600 !important;
    letter-spacing: 0.5px !important;
}
...
.tab-nav button {
    background: transparent !important;
    border: 1px solid var(--holo-tab-border) !important;
    color: var(--holo-tab-color) !important;
...
.hiresense-footer {
    text-align: center;
    padding: 30px;
    color: var(--holo-footer-color) !important;
    font-size: 0.8rem;
    letter-spacing: 2px;
    border-top: 1px solid var(--holo-footer-border);
    margin-top: 40px;
    position: relative;
}
...
.holo-grid {
    background-image:
        linear-gradient(var(--holo-grid-line) 1px, transparent 1px),
        linear-gradient(90deg, var(--holo-grid-line) 1px, transparent 1px);
...
/* ══════════════════════════════════════════════════════
   FOOTER LOGO BUTTON — FIXED POSITION
   ══════════════════════════════════════════════════════ */
#hs-footer-logo-btn {
    position: fixed !important;
    ...
    transition: all 0.3s ease !important;
    background: var(--holo-logo-bg) !important;
    padding: 0 !important;
    display: flex !important;
    align-items: center !important;
    justify-content: center !important;
...
(function () {
    ...
    var ThemeEngine = {
        ...
        init: function () {
            ...
            this.current = saved ? saved : 'system';
        ...
        apply: function (theme) {
            ...
            var resolved = (theme === 'system') ? this.getSystemTheme() : theme;
            document.documentElement.setAttribute('data-theme', resolved);
            document.body.setAttribute('data-theme', resolved);

            /* Keep Gradio's own built-in dark-mode class in sync.
               Gradio toggles `.dark` on <html>/<body> independently
               of our custom attribute, and some native components
               key their styling off that class. Without this, our
               toggle would only repaint the elements we explicitly
               overrode while the rest of the UI stayed on whatever
               Gradio picked at page load — which is why clicking
               "Dark" looked like it did nothing. */
            document.documentElement.classList.toggle('dark', resolved === 'dark');
            document.body.classList.toggle('dark', resolved === 'dark');

            /* Update active state on option buttons */
            var opts = document.querySelectorAll('.hs-theme-opt');
            for (var i = 0; i < opts.length; i++) {
                opts[i].classList.remove('active');
                if (opts[i].getAttribute('data-theme-val') === theme) {
    ...
    function createThemeToggle() {
        ...
        wrapper.id  = 'hs-theme-wrapper';

        /* Icon trigger button */
        var iconBtn = document.createElement('button');
        iconBtn.id        = 'hs-theme-icon-btn';
        iconBtn.title     = 'Theme settings';
        iconBtn.innerHTML = '☀️';

        /* Slide-down panel */
        var panel = document.createElement('div');
        panel.id  = 'hs-theme-panel';
        ...
        themes.forEach(function (t) {
            ...
            opt.className = 'hs-theme-opt';
            opt.setAttribute('data-theme-val', t.val);
            opt.textContent = t.label;
            opt.addEventListener('click', function (e) {
                e.stopPropagation();
                ThemeEngine.apply(t.val);
        ...
        /* System theme live change listener */
        if (window.matchMedia) {
            window.matchMedia('(prefers-color-scheme: light)')
                .addEventListener('change', function () {
                    if (ThemeEngine.current === 'system') {
                        ThemeEngine.apply('system');
                    }
                });
        }

        /* Apply saved/default theme — also marks active button */
    ...
    var Voice = {
        ...
        pickVoice: function () {
            ...
            for (var k = 0; k < this.voiceList.length; k++) {
                if (this.voiceList[k].lang &&
                    this.voiceList[k].lang.startsWith('en')) {
    ...
    function createWelcomeOverlay() {
        ...
        overlay.innerHTML = [
            ...
            '  <h1 id="hs-welcome-title" font="80px">HireSense_AI</h1>',
...
def build_footer_html(logo_b64: str) -> str:
    return (
        ...
        'HIRESENSE_AI &nbsp;|&nbsp; REDROB AI HACKATHON 2026 &nbsp;|&nbsp; '
        'PRODUCTION RANKING SYSTEM &nbsp;|&nbsp; '
        'BUILT BY SHAIK ARSHAD WASIB &nbsp;|&nbsp; SOLO PROJECT - TEAM LEADER'
        '</div>'
    )
...
<div class="stats-grid">
    ...
    <div class="stat-card">
        ...
        <span class="stat-label">GOOD MATCH</span>
    </div>
    <div class="stat-card">
        <span class="stat-number" style="color:#7b2ff7 !important;">{moderate_match}</span>
        <span class="stat-label">MODERATE MATCH</span>
    </div>
</div>
<div style="text-align:center;color:rgba(0,212,255,0.7);
...
if __name__ == "__main__":
    ...
    demo.launch(
        ...
        show_error=True,
        css=HOLOGRAPHIC_CSS,
        head=build_speech_head(LOGO_B64),
        js=JS_ON_LOAD,
        ssr_mode=False,
